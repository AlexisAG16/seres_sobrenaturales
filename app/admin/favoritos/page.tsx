...
type Favorite={_id:unknown;nombre_comun:string;tipo:string};
type FavoriteUser={_id:unknown;username:string;rol:string;favoritos:Favorite[]};
...
export default async function AdminFavoritos(){
  ...
  const users=await User.find({rol:"user"})
    ...
    .populate("favoritos","nombre_comun tipo")
    .sort({username:1})
    .lean() as unknown as FavoriteUser[];
