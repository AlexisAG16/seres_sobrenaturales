...
export default async function Historia(){
  ...
  return <section className="section prose history-page">
    ...
    <p className="lead">{user?"Pulsa el icono de cada crónica para descargar su edición Word.":"Puedes conocer las crónicas disponibles. Inicia sesión para acceder a los documentos completos y descargarlos."}</p>
    {!user&&<div className="panel access-callout"><h2>Contenido para miembros</h2><p>Las historias completas y sus archivos Word están disponibles para usuarios registrados.</p><div className="admin-actions"><Link className="button" href="/login?next=/historia">Iniciar sesión</Link><Link className="button secondary" href="/registro">Crear cuenta</Link></div></div>}
    <div className="history-library">{documents.map(document=><article className="history-book" key={document.file}>
      ...
      <div className="history-content"><h2>{document.title}</h2><p>{document.description}</p>
        {user?<DownloadDocumentButton file={document.file} title={document.title}/>:<Link className="download-word locked" href="/login?next=/historia"><DownloadIcon/><span><strong>Inicia sesión</strong><small>Para acceder y descargar</small></span></Link>}
      </div>
    </article>)}</div>
    <div className="panel history-note"><h2>Una colección en crecimiento</h2><p>Los documentos conservan las historias y genealogías del proyecto original con un nuevo diseño editorial.</p></div>
  </section>
}
